...
try:
    ...
    # ``_cuda_plugin`` is only present in pyscfadlib builds that ship the CUDA
    # plugin shim. On CPU-only installations it may be absent; degrade
    # gracefully so the (default) libcint path keeps working.
