...
class QMMM:
    ...
    def get_mm_ewald_pot(self, param=None):
        ...
        (ewovrl0, ewovrl1, ewovrl2) = (
            ...
            numpy.zeros((len(coords1), 3, 3), dtype=fx)
        )
    ...
    def get_qm_ewald_hess(self):
        ...
        ewself02 = numpy.zeros((len(coords1), len(coords1), 3, 3), dtype=fx)
        ...
        eye = numpy.eye(len(coords1), dtype=fx)
        # -d^2 Eself / dQi dQj
        ewself00 += -eye * 2 * ew_eta / \
            numpy.sqrt(numpy.pi)  # to include the lim
        # -d^2 Eself / dDia dDjb
        ewself11 += -numpy.einsum('ij,ab->ijab', eye, numpy.eye(3, dtype=fx)) \
