...
def tot_valence_electrons(mol: MolePad, charge: int = None, nkpts: int = 1) -> Array:
    if charge is None:
        charge = mol.charge
...
class XTB(XTBBase, SCFPad):
    ...
    def dip_moment(self, mol=None, dm=None, unit="Debye", origin=None, verbose=None, charges=None):
        if mol is None:
            mol = self.mol
