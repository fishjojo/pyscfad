...
def add_mm_charges(xtb_method, mm_coords, a, mm_charges, mm_radii, ewald_precision=1e-6, pbcqm=True, unit=None):
    if unit is None:
...
class QMMM:
    def __init__(self, method, mm_coords, a, mm_charges, mm_radii, pbcqm=True, ewald_precision=1e-6):
        '''
    ...
    get_Gv_weights = cell.get_Gv_weights
    ...
    def get_ewald_params(self, precision=None):
        ...
        e = precision
        Q = numpy.sum(self.mm_charges**2) + numpy.sum(self.mol.atom_charges()**2)
        eta = 1 / rcut * numpy.sqrt(
            1.5 * 
            lambertw(
                stop_gradient(2/3 * (4/e*Q/rcut/self.vol)**(2/3) * rcut**2)
                ).real
            )
        L = self.vol**(1/3)
        kmax = 1.73205081*eta/2/numpy.pi * numpy.sqrt(
            lambertw( stop_gradient(4*Q**(2/3)/3/numpy.pi**(2/3)/L**2/eta**(2/3) / e**(4/3)) ).real)
        mesh = numpy.asarray(numpy.ceil(numpy.diag(self.a) * kmax) * 2 + 1, dtype=int)
    ...
    def get_mm_ewald_pot(self, param=None):
        ...
        ewovrl0 = numpy.zeros_like(param.gam)
        ewovrl1 = numpy.zeros((len(coords1), 3))
        ewovrl2 = numpy.zeros((len(coords1), 3, 3))
        ...
        for i0, i1 in lib.prange(0, len(coords2), blksize):
            ...
            expnts = 2. / (1 / (param.gam*param.lgam)[:,None] + self.mm_radii[i0:i1][None])
            Tij = erfc(expnts * r[atom_to_bas]) / r[atom_to_bas]
            ewovrl0 -= numpy.einsum('ij,j->i', Tij, self.mm_charges[i0:i1])
            if param.dipgam is not None:
                expnts = 2. / (1 / param.dipgam[:,None] + self.mm_radii[i0:i1][None])
                ekR = numpy.exp(-expnts**2 * r**2)
                Tij = erfc(expnts * r) / r
                invr3 = (Tij + 2/numpy.sqrt(numpy.pi) * expnts * ekR) / r**2
                Tija = -numpy.einsum('ijx,ij->ijx', R, invr3)
                ewovrl1 -= numpy.einsum('j,ija->ia', self.mm_charges[i0:i1], Tija)
            if param.quadgam is not None:
                expnts = 2. / (1 / param.quadgam[:,None] + self.mm_radii[i0:i1][None])
                ekR = numpy.exp(-expnts**2 * r**2)
                Tij = erfc(expnts * r) / r
                invr3 = (Tij + 2/numpy.sqrt(numpy.pi) * expnts * ekR) / r**2
                Tija = -numpy.einsum('ijx,ij->ijx', R, invr3)
                ...
                ewovrl2 -= numpy.einsum('j,ijab->iab', self.mm_charges[i0:i1], Tijab) / 3
            ...
            ewovrl0 += numpy.einsum('ij,j->i', Tij, self.mm_charges[i0:i1])[atom_to_bas]
            if param.dipgam is not None:
                ewovrl1 += numpy.einsum('j,ija->ia', self.mm_charges[i0:i1], Tija)
            if param.quadgam is not None:
                ewovrl2 += numpy.einsum('j,ijab->iab', self.mm_charges[i0:i1], Tijab) / 3
        ...
        if param.dipgam is not None:
            ...
            ewg1  = numpy.einsum('gx,ig,g,g->ix', Gv, cosGvR1, zsinGvR2, Gpref, optimize=p)
            ewg1 -= numpy.einsum('gx,ig,g,g->ix', Gv, sinGvR1, zcosGvR2, Gpref, optimize=p)
        else:
            ewg1 = 0.
        # qm quad - mm pc
        if param.quadgam is not None:
            p = ['einsum_path', (3, 4), (0, 3), (0, 2), (0, 1)]
            ewg2  = -numpy.einsum('gx,gy,ig,g,g->ixy', Gv, Gv, cosGvR1, zcosGvR2, Gpref, optimize=p)
            ewg2 += -numpy.einsum('gx,gy,ig,g,g->ixy', Gv, Gv, sinGvR1, zsinGvR2, Gpref, optimize=p)
            ewg2 /= 3
        else:
            ewg2 = 0.
    ...
    def get_qm_ewald_hess(self):
        ...
        # = 1 - erf - 1 = -erf
        r = numpy.linalg.norm(coords1[:,None] - coords1[None,:] + 1e-100, axis=-1)
        ewself00 -= erf(ew_eta * r) / r
    
        # spurious interaction with Ewald compensenting gaussian charge
        # -d^2 Eself / dQi dQj
        #ewself00 += -numpy.eye(len(coords1)) * 2 * ew_eta / numpy.sqrt(numpy.pi)
        # -d^2 Eself / dDia dDjb
        ewself11 += -numpy.einsum('ij,ab->ijab', numpy.eye(len(coords1)), numpy.eye(3)) \
        ...
        # qm dip - qm dip
        ewg11  = numpy.einsum('gx,gy,ig,jg,g->ijxy', Gv, Gv, cosGvR, cosGvR, Gpref)
        ewg11 += numpy.einsum('gx,gy,ig,jg,g->ijxy', Gv, Gv, sinGvR, sinGvR, Gpref)
        # qm pc - qm quad
        ewg02  = -numpy.einsum('gx,gy,ig,jg,g->ijxy', Gv, Gv, cosGvR, cosGvR, Gpref)
        ewg02 += -numpy.einsum('gx,gy,ig,jg,g->ijxy', Gv, Gv, sinGvR, sinGvR, Gpref)
        ewg02 /= 3
    ...
    def get_vdiff(self, mol, ewald_pot):
        ...
        for iatm in range(mol.natm):
            ...
            if self.param.dipgam is not None:
                ...
                vdiff = vdiff.at[:,p0:p1].subtract(numpy.einsum('x,xuv->uv', v1, s1r[iatm]))
            if self.param.quadgam is not None:
                #vdiff[:,p0:p1] -= numpy.einsum('xy,xyuv->uv', v2, s1rr[iatm])
                vdiff = vdiff.at[:,p0:p1].subtract(numpy.einsum('xy,xyuv->uv', v2, s1rr[iatm]))
    ...
    def get_veff(self, mol=None, dm=None, dm_last=0, vhf_last=0, hermi=1, s1e=None,
        ...
        ediff = self.energy_ewald(dm=dm, mm_ewald_pot=mm_ewald_pot, qm_ewald_pot=qm_ewald_pot)
    ...
    def energy_ewald(self, dm=None, mm_ewald_pot=None, qm_ewald_pot=None):
        ...
        e  = numpy.einsum('i,i->', ewald_pot, self.get_qm_charges(dm))
        if self.param.dipgam is not None:
        ...
        if self.param.quadgam is not None:
            ...
            e += numpy.einsum('ixy,ixy->', ewald_pot, self.get_qm_quadrupoles(dm))
