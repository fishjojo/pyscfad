...
static void fill_ij_r0_s2_igtj(double *vjp, double *in, double* ybar, int comp,
                               int ip, size_t nij, int di, int dj, int dk)
{
...
#pragma omp parallel
{
        ...
        #pragma omp single
        {
                vjp_bufs = malloc(sizeof(double *) * nthreads);
        }
        double *vjp_loc;
        if (thread_id == 0) {
                vjp_loc = vjp;
        ...
        omp_dsum_reduce_inplace(vjp_bufs, natm_ij*comp);
        if (thread_id != 0) {
                free(vjp_loc);
        }
}
