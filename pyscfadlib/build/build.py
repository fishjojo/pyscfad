...
EPILOG = """
  From the root directory of pyscfadlib, run
  `python build/build.py build --wheels=<list of wheels>`.
...
WHEEL_BUILD_TARGET_DICT = {
    "pyscfad-cuda-plugin": "//pyscfadlib/tools:build_cuda_kernels_wheel",
}
...
def add_artifact_subcommand_arguments(parser: argparse.ArgumentParser):
  ...
  parser.add_argument(
      "--wheels",
      type=str,
      default="pyscfad-cuda-plugin",
      help=
        """
        A comma separated list of wheels to build.
        Valid options are: pyscfad-cuda-plugin
        """,
  )
  ...
  parser.add_argument(
      ...
      help="Directory to which the wheel should be written.",
  )
  ...
  parser.add_argument(
    ...
    action="store_true",
    help="""
      If true, writes the Bazel options to the .pyscfad_configure.bazelrc file but
  ...
  compile_group.add_argument(
      ...
      nargs="?",
      help="""
        Whether to use Clang as the compiler.
        """,
  )
...
async def main():
  parser = argparse.ArgumentParser(
      description=r"""
        CLI for building wheel packages and for updating the
        requirements_lock.txt files
        """,
      epilog=EPILOG,
  ...
  build_artifact_parser = subparsers.add_parser(
      "build", help="Builds the artifact"
  )
  ...
  if args.use_clang:
    raise KeyError("Clang not supported.")
  else:
  ...
  if args.target_cpu_features == "release":
    if arch in ["x86_64", "AMD64"]:
      logging.debug(
          "Using release cpu features: --config=avx_posix"
      )
      wheel_build_command_base.append("--config=avx_posix")
  ...
  with open(".pyscfad_configure.bazelrc", "w") as f:
    pyscfad_configure_options = utils.get_jax_configure_bazel_options(wheel_build_command_base.get_command_as_list(), False)
    if not pyscfad_configure_options:
      logging.error("Error retrieving the Bazel options to be written to .pyscfad_configure.bazelrc, exiting.")
      sys.exit(1)
    f.write(pyscfad_configure_options)
    logging.info("Bazel options written to .pyscfad_configure.bazelrc")
  ...
  if args.configure_only:
    ...
    for wheel in args.wheels.split(","):
      ...
      if "plugin" in wheel and "pyscfad" not in wheel:
        wheel = "pyscfad-" + wheel

      if wheel not in wheel_build_targets.keys():
        logging.error(
            "Incorrect wheel name provided, valid choices are: "
            "pyscfad-cuda-plugin,"
        )
        sys.exit(1)
      ...
      wheel_build_command.append(f"--pyscfadlib_git_hash={git_hash}")
